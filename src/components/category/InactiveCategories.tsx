...
interface InactiveCategoriesProps {
  onAddCategory: () => void;
  onEdit: (id: number) => void;
}

const InactiveCategories: React.FC<InactiveCategoriesProps> = ({ onAddCategory, onEdit }) => {
  ...
  const fetchInactiveCategories = async () => {
    ...
    try {
      ...
      if (response && response.data) {
        // Filter only inactive categories and transform API response
        const inactiveCategories = response.data
          .filter((category: any) => category.status === 'inactive' || category.status === false)
          .map((category: any) => ({
            id: category.id || category._id,
            name: category.name || category.categoryName,
            description: category.description || category.categoryDescription || '',
            status: false, // All are inactive since we filtered
      ...
      setError(error.message || 'Failed to fetch inactive categories');
  ...
  const handleStatusToggle = async (id: number) => {
    try {
      ...
      // await updateCategory(id.toString(), { status: 'active' });
      
      // Remove from inactive categories list since it's now active
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 sm:p-6 lg:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center h-64">
            ...
            <span className="ml-3 text-gray-600">Loading inactive categories...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
            <p>Error: {error}</p>
            <button 
              onClick={fetchInactiveCategories}
        ...
        {categories.length === 0 && !loading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No inactive categories found</h3>
            <p className="text-gray-500 mb-4">All categories are currently active or none exist.</p>
            <button
